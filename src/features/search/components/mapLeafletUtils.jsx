  ...
  return L.divIcon({
    ...
    html: `
      <div class="group relative flex flex-col items-center">
        <svg class="w-8 h-8 text-primary-500 wiggle drop-shadow-md" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div class="mini-card absolute left-1/2 -translate-x-1/2 -top-32 opacity-0 transition-opacity duration-200" style="z-index:10000;">
          <div class="w-36 md:w-52 bg-white rounded-xl z-[9999] overflow-hidden font-sans shadow-lg">
            <img src="${image}" alt="${escapeHtml(house.name)}" class="w-full h-24 md:h-40 object-cover" />
            <div class="p-2 space-y-1 font-sans">
              <div class="flex items-center justify-between gap-1">
                <span class="text-[10px] sm:text-[16px] font-medium truncate">${escapeHtml(house.name)}</span>
                ${voteHTML}
              </div>
              <div class="text-[9px] sm:text-[14px] text-primary-600 font-semibold">
